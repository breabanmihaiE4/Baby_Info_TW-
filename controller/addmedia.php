...
<body>
    ...
    <div class="returnbtn">
        <?php
            echo'<form action="../view/multimedia_view.php?id='.$_COOKIE['cookieChildId'].'" method="POST" class="retrun">' 
        ?>
...
<?php
        ...
        if(isset($_POST['incarcare'])){
            ...
            if(isset($_FILES['img']) && $_FILES['img']['error'] === UPLOAD_ERR_OK) {
                ...
                $sql = "insert into multimedia (IdCopil, media_path) values ('$childId', '$photoPath')";
                $result = mysqli_query($connect, $sql);
                if($result){
                    echo "<script>alert('Photo uploaded!');</script>" . $targetFile;
                    mysqli_close($connect);
                    header("Location: ../view/multimedia_view.php?id=$childId","refresh:0"); 
                    exit();
                } else {
                    echo "Error upload!";
                }
