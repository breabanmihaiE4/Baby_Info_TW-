...
if (isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $updateSql = "UPDATE medical SET title = '$title', description = '$description', event_date = DATE('$eventDate') WHERE id = '$eventId'";

        if (mysqli_query($connect, $updateSql)) {
            // Redirecționați către pagina timeline-ului sau la pagina evenimentului editat
            header("Location: ../view/medical_view.php?id=$cookieId");
            exit();
        } else {
        ...
        require '../model/_dbcon.php';
        $sql = "SELECT * FROM medical WHERE id = '$eventId'";
